...
export default function Address({ props = {
    ...
    const validationSchema = Yup.object().shape({
        ...
        phoneNumber: Yup.string()
            ...
            .strict(true),
        provine: Yup.string()
            .required(USER_MESSAGES.PROVINE_MISSING)
            .strict(true),
        district: Yup.string()
            .required(USER_MESSAGES.DISTRICT_MISSING)
            .strict(true),
        ward: Yup.string()
            .required(USER_MESSAGES.WARD_MISSING)
            .strict(true),
    });
                                                        ...
                                                        >
                                                            {
                                                                provine !== "" ? (
                                                                    (() => {
                                                                        const index = LIST_ADDRESSES_VN.map(item => item.name).indexOf(provine);
                                                                        return LIST_ADDRESSES_VN[index]?.districts.map(item => (
                                                                            <MenuItem key={item.name} value={item.name}>{item.name}</MenuItem>
                                                                        ));
                                                                    })()
                                                                ) : null
                                                        ...
                                                        >
                                                            {
                                                                provine !== "" && district !== "" ? (
                                                                    (() => {
                                                                        const indexProvine = LIST_ADDRESSES_VN.map(item => item.name).indexOf(provine);
                                                                        const indexDistrict = LIST_ADDRESSES_VN[indexProvine]?.districts.map(item => item.name).indexOf(district);
                                                                        return LIST_ADDRESSES_VN[indexProvine]?.districts[indexDistrict]?.wards.map(item => (
                                                                            <MenuItem key={item.name} value={item.name}>{item.name}</MenuItem>
                                                                        ));
                                                                    })()
                                                                ) : null
